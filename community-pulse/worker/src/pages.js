...
const API='${API}';
let jwt=localStorage.getItem('verify_jwt')||null;
...
async function init(){
  ...
  if(jwt){try{const p=await api('GET','/api/verify/me');if(p.identity_hash){currentProfile=p;showDashboard(p);return}else{console.log('/me response:',JSON.stringify(p))}}catch(e){console.error('/me failed:',e)}jwt=null;localStorage.removeItem('verify_jwt')}
...
async function finishPasskey(){
  ...
  try{
    ...
    const registration=await client.register({user:'Marblehead Verified Resident',challenge:bc});
    ...
    jwt=pk.token;localStorage.setItem('verify_jwt',jwt);
    pendingReg=null;
...
document.getElementById('btn-add-device').addEventListener('click',async()=>{
  ...
  const reg=await client.register({user:'Marblehead Verified Resident',challenge:bc});
...
window.signIn=async function(){
  ...
  try{
    ...
    if(r.ok&&r.token){jwt=r.token;localStorage.setItem('verify_jwt',jwt);const p=await api('GET','/api/verify/me');if(p.identity_hash){currentProfile=p;showDashboard(p)}}
  }catch(e){
    if(signInBtn){signInBtn.disabled=false;signInBtn.textContent='Sign in'}
...
document.getElementById('btn-signout').addEventListener('click',()=>{jwt=null;currentProfile=null;localStorage.removeItem('verify_jwt');hide('state-dashboard');hide('state-registered');hide('state-invite');hide('state-loading');show('state-landing');['reg-status','inv-status','device-status'].forEach(id=>hide(id))});
